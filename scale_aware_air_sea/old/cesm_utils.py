...
def load_and_combine_cesm(
    ...
    mapper = fsspec.get_mapper(
        "https://ncsa.osn.xsede.org/Pangeo/pangeo-forge-test/prod/recipe-run-502/pangeo-forge/cesm-atm-025deg-feedstock/cesm-atm-025deg.zarr"
    )
    ...
    ocean_grid = ds_ocean.isel(time=0).drop_vars(
        [co for co in ds_ocean.coords if co not in ["TLONG", "TLAT"]]
    )
    ...
    regridder = xe.Regridder(
        atmos_grid, ocean_grid, "bilinear", weights=ds_regridder, periodic=True
    )
    ds_atmos_regridded = regridder(
        ds_atmos[["TS", "UBOT", "VBOT", "QREFHT", "PSL", "U10", "TREFHT"]]
    )

    # Combine into merged dataset
    ds_merged = xr.merge(
        [
            ds_atmos_regridded.chunk(
                {"time": 1}
            ),  # to have more manageable sized chunks (same as ocean variables)
            ds_ocean[
                ["SST", "u_ocean", "v_ocean"]
            ],  # .chunk({'time':73}), # to have same chunksizes as atmospheric variables
        ]
    )
    ...
    for mask_coord in ["TAREA"]:
        ...
        ds_merged.coords[mask_coord] = (
            ds_merged[mask_coord].where(mask, 0.0).astype(np.float64)
        )
