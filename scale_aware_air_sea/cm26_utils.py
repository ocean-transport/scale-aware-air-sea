...
def load_and_combine_cm26(filesystem: gcsfs.GCSFileSystem, inline_array=False)-> xr.Dataset:
    ...
    ds_ocean['u_ocean'] = grid.interp_like(ds_ocean['usurf'].fillna(0), ds_ocean['surface_temp']).where(sst_wet_mask)
    ds_ocean['v_ocean'] = grid.interp_like(ds_ocean['vsurf'].fillna(0), ds_ocean['surface_temp']).where(sst_wet_mask)
    # FIXME: All of these additional operations (filling/masking) add a lot more tasks...
    ...
    path = 'gs://leap-persistent/jbusecke/scale-aware-air-sea/regridding_weights/CM26_atmos2ocean.zarr'
