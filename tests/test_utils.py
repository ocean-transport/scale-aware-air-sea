    ...
    ds_test = xr.Dataset(
        {
            k: xr.DataArray(
                data,
                dims=["x", "y", "time"],
                coords={"time": range(nt), "x": x, "y": y, "lon": lon, "lat": lat},
            )
            for k in ["a", "b"]
        },
    )
...
class Test_weighted_coarsen:
    def test_simple_2_x_2(self):
        ...
        ds = xr.DataArray(
            data_full, coords={"area": (["x", "y"], weights_full)}, dims=["x", "y"]
    ...
    def test_nan_mismatch_variables(self):
        ...
        with pytest.raises(
            ValueError, match="Found variables with non-matching missing values."
        ):
    ...
    def test_nan_mismatch_weights(self):
        ...
        with pytest.raises(
            ValueError,
            match="Missing values in variables are not matching locations of <=0 values in weights array.",
        ):
            weighted_coarsen(ds, {"x": 2, "y": 2}, "area")
    ...
    def test_weights_nan(self):
        ...
        with pytest.raises(
            ValueError, match="Found missing values in weights coordinate "
        ):
            weighted_coarsen(ds, {"x": 2, "y": 2}, "area")
